...
function BlockGroottesFullProblem(k, d, t)
    PWithQList = CreateRelevantPQiPartitions(k,d,t) #CreateRelevantPQiPartitions(k,d,t)
...
#BlockGroottesFullProblem(8, 8, 4)

function BlockGroottesFullProblemPlusHalf(k, d, t)
    PWithQList =  CreatePQiPartitions(k,d,t+1) #CreatePQiPartitions(k,d,t+1)
...
#BlockGroottesFullProblemPlusHalf(7, 5, 5)
#BlockGroottesFullProblemPlusHalf(4, 6, 6)
...
##Use this one if sign is constant on column representative set (is the case in full reduction)
    ...
    #we do the normal t-th level version. The corresponding representative set elements must be given as input arguments!
        ...
        for (tempmonoomK, valueK)  in RowColKDict
            ...
            for i in unique(tempmonoomK)
                ...
                # println("monoom ",tempmonoomK)
                # println("Prow ", Prow)
                # println("Pcol ", Pcol)
                # println("IndexSet ", IndexSet)
                # println("ReprDRow ", ReprDRow)
                # println("ReprDCol ", ReprDCol)
...
## use this one if also sign on column representative set needs to be taken into account. WITHOUT SPEED-UP-TRICKS, BUT READABLE!
