...
include("FastNonCommutativeRepresentativeSet05-3.jl")
...
function MUBWriteSDPASymBlockDiagFULL(k,dim,t, option=1)
    ...
    @time for (MultiPartition, CorrespondingRows) in BlocksElement
        ...
        for rowindex =1:blockSize
            ...
            for colindex=rowindex:blockSize
                #compute the inner product.
                # println("rijreprset: ", ReprSetArray[rowindex])
                # println("colreprset: ", ReprColSetArray[colindex])
                TotalInnerProducts = ReduceInnerProductFULLNEW(ReprSetArray[rowindex], ReprColSetArray[colindex])

                # if blockindex == 3
                #     println("Total inner products: ", TotalInnerProducts)
                # end
                ...
                for (monomialpair,value) in TotalInnerProducts 
                    ...
                    if !haskey(MonomialValuesDictionary,monomialpair)
                        newmonomial::SymEngine.Basic = bepaalwaardemonoom(dim,deepcopy(tempmonoomDim),deepcopy(tempmonoomK))
    ...
    #println("number of zero rows: ", numzerorows)
    println("##### Collecting the variables occuring in the blocks...")
    variables = [];
    @time for outerindex=1:size(ReducedBlocks,1)
         for i=1:size(ReducedBlocks[outerindex],1)
...
function MUBTESTSept21(k,dim,t, option=1)
    manual_epsilon = 0.0000000000000001  #if smaller than this epsilon, consider coefficient to be zero.
    smallbigfloat = BigFloat("0.0000000000000000000000000000001")
    ...
    for rowIdx=1:FullSize
        for colIdx=1:FullSize
    ...
    for blockSize in BlockSizes 
        ...
        for bridx=1:blockSize
            for bcidx = 1:blockSize
    ...
    #             # println("rijreprset: ", ReprSetArray[rowindex])
    #             # println("colreprset: ", ReprColSetArray[colindex])
    #             TotalInnerProducts = ReduceInnerProductFULLTOTAL(ReprSetArray[rowindex], ReprColSetArray[colindex])
...
#@time MUBWriteSDPASymBlockDiagFULL(4,6,3,1)
MUBTESTSept21(3,3,4,1)
#@time MUBWriteSDPASymBlockDiagFULL(4,4,4,2)
