            ...
            if !isempty(Ivec_of_basis_single)
                ...
                if (length(Ivec_occurring_unique) > 1 && Ivec[index1] == maximumI)
                    ...
                    NewVar, DictValueMon = DetValMon(
                        ...
                        Ipart1,
                        Kpart1,
                        coeff * (1 / Double64(d + 1 - length(Ivec_occurring_unique))),
                    )
                    #Minus the others:
                    for basiselt in Ivec_occurring_unique
                        TempDictionary = Dict{Tuple{Vector{Int}, Vector{Int}}, Double64}()
                    ...
                    # return value_to_return/Double64(d+1-length(Ivec_occurring_unique));
                    # return VarSet,DictValueMon = DetValMon(VarSet,DictValueMon,d,Ivec,Kvec,coeff/Double64(d+1-length(Ivec_occurring_unique)); #I have modified the coefficient above instead
                    return NewVar, DictValueMon
                end
            end
        end
